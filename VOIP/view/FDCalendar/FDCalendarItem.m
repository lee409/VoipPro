...
@property (copy, nonatomic) NSString *nianStr;
@property (copy, nonatomic) NSString *yueStr;
...
- (void)setDate:(NSDate *)date {
    _date = date;
    self.nianStr = [self dateStr:date str:@"YYYY"];
    self.yueStr = [self dateStr:date str:@"M"];
    //self.dateStr = [self dateStr:date str:@"d"];
    
    [self.collectionView reloadData];
}
-(NSString*)dateStr:(NSDate *)date str:(NSString*)str{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.dayLabel.textColor = CHANGEColorWith(0x4cafff,1.0);
    cell.chineseDayLabel.textColor = CHANGEColorWith(0x4cafff,1.0);
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        if (self.signTime) {
            for (int i = 0; i < self.signTime.count; i++) {
                NSArray*resustOut = [self.signTime[i] componentsSeparatedByString:@"-"];
                for (int j = 0; j < resustOut.count; j++)
                    if (![self.nianStr isEqualToString:resustOut[0]]) {
                        break;
                    }else if (![self.yueStr isEqualToString:resustOut[1]]){
                        break;
                    }else{
                        if ([cell.dayLabel.text isEqualToString:resustOut[2]]) {
                            UIImageView*imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"me_sign_icon_circle"]];
...
//            cell.layer.cornerRadius = cell.frame.size.height / 2;
            ...
            cell.chineseDayLabel.textColor = [UIColor purpleColor];
        }
